...
public class ImpactReportMapper {
  ...
  public ImpactReport toImpactReport(
    ...
    final List<ImpactedTestEntry> impactedTests =
        relevantTestsWithCauses.entrySet().stream()
            .map(
                entry ->
                    new ImpactedTestEntry(
                        normalizedProjectPath.relativize(
    ...
    final List<CauseSummaryEntry> topCauses =
        causeCounts.entrySet().stream()
            ...
            .sorted(
                Comparator.comparingInt(CauseSummaryEntry::impactedTestCount)
                    .reversed()
  ...
  private ImpactReportMetadata buildMetadata(
    ...
    final List<String> sortedAnnotations =
        Optional.ofNullable(impactCheckerConfig.getAnnotations()).orElse(List.of()).stream()
            .sorted()
            .toList();
  ...
  private Map<String, Integer> buildCauseCounts(final List<ImpactedTestEntry> impactedTests) {
    ...
    impactedTests.forEach(
        entry -> entry.causes().forEach(cause -> causeCounts.merge(cause, 1, Integer::sum)));
    return causeCounts;
  }
  ...
  private ImpactGraph buildImpactGraph(
    ...
    final List<TypeCandidate> typeCandidates =
        buildTypeCandidates(impactedTypeToCauses, causeCounts, selectedCauses);
    final Set<String> selectedTypes = selectTypes(typeCandidates, selectedCauses.size());
    final List<TestCandidate> testCandidates =
        buildTestCandidates(impactedTests, causeCounts, selectedCauses);
    final Set<String> selectedTests =
        selectTests(testCandidates, selectedCauses.size(), selectedTypes.size());

    final List<ImpactGraphNode> causeNodes =
        selectedCauses.stream()
            .map(
                cause ->
                    new ImpactGraphNode(
                        nodeId(ImpactGraphNodeKind.CAUSE, cause),
                        cause,
                        ImpactGraphNodeKind.CAUSE,
                        causeCounts.getOrDefault(cause, 0)))
            ...
            .map(
                type ->
                    new ImpactGraphNode(
                        ...
                        typeCandidates.stream()
                            .filter(candidate -> candidate.type().equals(type))
                            .findFirst()
                            .map(TypeCandidate::score)
                            .orElse(0)))
            .sorted(graphNodeComparator())
            .toList();
            ...
            .map(
                test ->
                    new ImpactGraphNode(
                        ...
                        toTestClassName(test),
                        ImpactGraphNodeKind.TEST,
                        testCandidates.stream()
                            .filter(candidate -> candidate.relativeTestPath().equals(test))
                            .findFirst()
                            .map(TestCandidate::score)
                            .orElse(0)))
            .sorted(graphNodeComparator())
            .toList();

    final List<ImpactGraphEdge> edges =
        buildEdges(
            impactedTests, impactedTypeToCauses, selectedCauses, selectedTypes, selectedTests);

    final int totalNodes =
        countTotalNodes(causeCounts.keySet(), impactedTypeToCauses.keySet(), impactedTests);
    final int totalEdges =
        countTotalEdges(impactedTests, impactedTypeToCauses, causeCounts.keySet());
  ...
  private ImpactGraphBundle buildGraphBundle(
      final ImpactGraph fullGraph, final List<CauseSummaryEntry> topCauses) {
  ...
  private ImpactGraph buildOverviewGraph(final ImpactGraph fullGraph) {
    ...
    for (final ImpactGraphEdge edge : fullGraph.edges()) {
      ...
      if (fromNode.kind() == ImpactGraphNodeKind.CAUSE
          && toNode.kind() == ImpactGraphNodeKind.TYPE) {
      ...
      if (fromNode.kind() == ImpactGraphNodeKind.TYPE
          ...
          && typeToSelectedTestEdges.getOrDefault(fromNode.id(), 0)
              < MAX_OVERVIEW_TEST_EDGES_PER_TYPE) {
        selectedEdges.add(edge);
        typeToSelectedTestEdges.merge(fromNode.id(), 1, Integer::sum);
      }
    }

    overviewEdges.addAll(
        selectedEdges.stream()
            .sorted(
                Comparator.comparing(ImpactGraphEdge::fromNodeId)
                    .thenComparing(ImpactGraphEdge::toNodeId))
            .toList());
  ...
  private List<ImpactGraphSection> buildCauseSections(
    ...
    for (final CauseSummaryEntry causeEntry : topCauses) {
      ...
      final List<ImpactGraphNode> sectionNodes =
          fullGraph.nodes().stream()
              .filter(
                  node ->
                      node.id().equals(causeNodeId)
                          || typeIds.contains(node.id())
                          || testIds.contains(node.id()))
              .toList();
      final List<ImpactGraphEdge> sectionEdges =
          focusedEdges.stream()
              .sorted(
                  Comparator.comparing(ImpactGraphEdge::fromNodeId)
                      .thenComparing(ImpactGraphEdge::toNodeId))
            ...
            entry ->
                new TypeCandidate(
                    ...
                    entry.getValue().stream()
                        ...
                        .sum()))
        .sorted(
            Comparator.comparingInt(TypeCandidate::score)
                .reversed()
                .thenComparing(TypeCandidate::type))
        .toList();
  }

  private Set<String> selectTypes(
      final List<TypeCandidate> typeCandidates, final int selectedCauseCount) {
    final Set<String> selected = new HashSet<>();
  ...
  private List<TestCandidate> buildTestCandidates(
    ...
    return impactedTests.stream()
        ...
        .sorted(
            Comparator.comparingInt(TestCandidate::score)
                .reversed()
  ...
  private List<ImpactGraphEdge> buildEdges(
    ...
    impactedTests.forEach(
        entry -> testToCauses.put(entry.relativeTestPath().toString(), Set.copyOf(entry.causes())));
