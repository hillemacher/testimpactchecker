...
public class JavaImpactUtils {
  ...
  public Set<Path> findRelevantTests(
    ...
    return new HashSet<>(
        findRelevantTestsWithCauses(changedFilePaths, implementedInterfaces, testDirPaths)
            .keySet());
  }
   ...
   *
   * <p>The returned cause set contains changed class names, even when a match occurred through an
   * implemented interface. This keeps downstream reporting stable and allows callers to map all
   * impacts back to the actual changed production classes.
   *
   * @param changedFilePaths changed class file paths (relative to repository root)
   * @param implementedInterfaces map of changed class paths to directly implemented interface names
   * @param testDirPaths directories containing Java test sources
  ...
  public Map<Path, Set<String>> findRelevantTestsWithCauses(
    ...
    final Set<String> changedClassNames =
        changedFilePaths.stream().map(this::toSimpleClassName).collect(Collectors.toSet());

    return createEngine()
        .analyzeTestsWithDirectModel(
            changedFilePaths,
   ...
   *
   * <p>This delegates Git diff resolution to {@link ChangedClassLocator}, then filters results to
   * Java files that are inside discovered {@code src/main/java} roots. Returned paths are
   ...
   *
   * <p>The interface names are captured as simple names because the current analyzer operates on
   * simple-name matching. This data is used both in direct matching and as seed types for
  ...
  public Map<Path, Set<String>> findImplementedInterfaces(
    ...
    return new ChangedTypeSeedResolver(javaParser)
   ...
   *
   * <p>Keeping engine construction in one place guarantees consistent behavior between direct and
   * transitive callers and avoids spreading object wiring across orchestrators.
   *
